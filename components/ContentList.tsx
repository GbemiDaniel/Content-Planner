...
const PostListItem: React.FC<{ post: Post; currentPostId?: string; onEdit: (id: string) => void; onDelete: (id: string) => void;}> = ({ post, currentPostId, onEdit, onDelete }) => {
    ...
    return (
        <div
            className={`p-3 rounded-lg transition-all duration-300 flex justify-between items-start ${
                currentPostId === post.id ? 'bg-purple-500/30' : 'bg-black/10 dark:bg-white/5 hover:bg-black/20 dark:hover:bg-white/10'
            }`}
        >
            <div className="flex-1 overflow-hidden">
                <p className="truncate font-medium">{post.content[0]?.text || "Untitled Post"}</p>
                <div className="flex items-center gap-2 mt-1">
                    <span className={`px-2 py-0.5 text-xs rounded-full ${READINESS_CONFIG[post.status].color}`}>
                        {post.status}
                    </span>
                    {renderTimeInfo()}
                </div>
            ...
            <div className="flex items-center gap-1 ml-2">
                <button onClick={() => onEdit(post.id)} className="p-2 rounded-full hover:bg-white/20 text-blue-400">
                    <Edit size={16} />
                </button>
                <button onClick={() => onDelete(post.id)} className="p-2 rounded-full hover:bg-white/20 text-red-400">
                    <Trash2 size={16} />
                </button>
            </div>
        </div>
